  ...
  <div class="card">
    <div class="card-body">
      <div class="row">
        <div class="col-md-6 bold">
          <h5 class="card-title mb-2">kardex Details</h5>
        </div>
      ...
      <table class="table text-center table-info mt-4" id="storekardexBook">
        <thead>
          <tr>
            <th scope="col">#</th>
        ...
        <tbody>
          <tr v-for="kardex in kardex_details.data" :key="kardex.id">
            <td>{{ kardex.product_id }}</td>

            <td>{{ convert_date(kardex.date) }}</td>

            <td>{{ kardex.item_name }}</td>

            <td>{{ kardex.description_item }}</td>

            <td>{{ kardex.quantity }}</td>
            <td>{{ kardex.price }}</td>

            <td>{{ kardex.invoice }}</td>

            <td>{{ kardex.remarks }}</td>
            ...
            <td>
              <button
                ...
                data-toggle="modal"
                data-target=".kardexPageview"
                @click="view_kardex(kardex)"
              >
                view
              </button>
            </td>
          </tr>
          <tr v-if="kardex_details.length == 0">
    ...
    <div class="card-footer">
      <pagination
        ...
        :limit="3"
        align="right"
        @pagination-change-page="get_kardex_details"
      ></pagination>
    </div>
    ...
    <div
      class="modal fade kardexPageview"
      tabindex="-1"
      role="dialog"
    ...
    >
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <kardex-details-view> </kardex-details-view>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    view_kardex(kardex) {
      bus.$emit("kardex-details", kardex);
    },

    get_kardex_details(page = 1) {
      axios
        .get("office-kardexdata?page=" + page, {
          params: {
        ...
        .then((response) => {
          this.kardex_details = response.data;

          //     $(document).ready(function () {
          //   $("#storekardexBook").DataTable({
          //     dom: "Bfrtip",
          //     buttons: ["copy", "csv", "excel", "pdf", "print"],
          //   });
          // });

          setTimeout(() => (this.kardex_details = response.data));
        });
    },
  },
};
